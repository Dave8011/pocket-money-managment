...
import { 
  Wallet, TrendingUp, TrendingDown, CreditCard, Plus, Trash2, 
  Bell, Clock, Home, List, Zap, CheckCircle, X, Droplet, 
  Landmark, PieChart as PieChartIcon, Download, LogOut, User,
  Eye, EyeOff, Pencil, PiggyBank, ArrowRight, Search
} from 'lucide-react';
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip as RechartsTooltip, Legend } from 'recharts';

// NOTE: Uncomment the line below ONLY if you have installed @capacitor/local-notifications
...
export default function App() {
  ...
  useEffect(() => {
    if (!user) return;
    const getColl = (name) => collection(db, 'artifacts', appId, 'users', user.uid, name);
    const unsubTxn = onSnapshot(getColl('transactions'), (snap) => setTransactions(snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a,b) => new Date(b.date) - new Date(a.date))));
    const unsubLoans = onSnapshot(getColl('loans'), (snap) => setLoans(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubBills = onSnapshot(getColl('bills'), (snap) => setBills(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubDues = onSnapshot(getColl('dues'), (snap) => setDues(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubMisc = onSnapshot(doc(getColl('settings'), 'misc'), (doc) => { if(doc.exists()) setMiscSettings(doc.data()); });
    return () => { unsubTxn(); unsubLoans(); unsubBills(); unsubDues(); unsubMisc(); };
  }, [user]);
  ...
  const getDaysRemaining = (dayOfMonth) => {
    ...
    if (today.getDate() > dayOfMonth) targetDate = new Date(currentYear, currentMonthIdx + 1, dayOfMonth);
    return Math.ceil((targetDate - today) / (1000 * 60 * 60 * 24));
  };

  const pendingPayments = useMemo(() => {
    const list = [];
    bills.forEach(bill => { if (!isPaidThisMonth(bill.lastPaidDate)) list.push({ type: 'bill', id: bill.id, title: bill.title, amount: bill.amount, daysLeft: getDaysRemaining(bill.dayOfMonth), meta: 'Monthly Bill' }); });
    loans.forEach(loan => { if (loan.pendingAmount > 0 && !isPaidThisMonth(loan.lastPaidDate)) list.push({ type: 'loan', id: loan.id, title: `EMI: ${loan.name}`, amount: loan.emiAmount, daysLeft: getDaysRemaining(loan.emiDay), meta: `Loan Bal: ${formatCurrency(loan.pendingAmount)}` }); });
    dues.forEach(due => { if (!due.isPaid) { const daysLeft = Math.ceil((new Date(due.dueDate) - new Date()) / (1000 * 60 * 60 * 24)); list.push({ type: 'due', id: due.id, title: due.title, amount: due.amount, daysLeft, meta: `Due Date: ${due.dueDate}` }); } });
    return list.sort((a,b) => a.daysLeft - b.daysLeft);
  }, [bills, loans, dues]);

  // --- NOTIFICATION LOGIC (Commented out to prevent crash if plugin missing) ---
  /*
  useEffect(() => {
    const checkAndNotify = async () => {
        if(pendingPayments.length > 0) {
                ...
                if(perm.display === 'granted') {
                    ...
                    if (lastNotified !== today) {
                        await LocalNotifications.schedule({
                            notifications: [{
                                title: "Payments Due",
                                body: `You have ${pendingPayments.length} payments pending.`,
                                id: 1,
            ...
            } catch (e) { console.log("Notif error - likely browser"); }
        }
    };
    checkAndNotify();
  ...
  */
  ...
  const currentMonthTxns = transactions.filter(t => { const d = new Date(t.date); return d.getMonth() === currentMonthIdx && d.getFullYear() === currentYear; });
  const totalIncome = transactions.reduce((acc, t) => t.type === 'income' ? acc + t.amount : 0, 0);
  const totalExpense = transactions.reduce((acc, t) => t.type === 'expense' ? acc + t.amount : 0, 0);
  const walletBalance = totalIncome - totalExpense;
  const monthIncome = currentMonthTxns.reduce((acc, t) => t.type === 'income' ? acc + t.amount : 0, 0);
  const totalCommitments = pendingPayments.reduce((acc, item) => acc + item.amount, 0);
  const freeCash = walletBalance - totalCommitments;
  const miscSpent = currentMonthTxns.filter(t => t.type === 'expense' && t.category === 'Miscellaneous').reduce((acc, t) => acc + t.amount, 0);
  const miscBalance = Math.max(0, (miscSettings.limit || 0) - miscSpent);

  const expenseData = useMemo(() => {
    const data = {};
    currentMonthTxns.filter(t => t.type === 'expense').forEach(t => { data[t.category] = (data[t.category] || 0) + t.amount; });
  ...
  const handlePayItem = async (item) => {
    ...
    await addDoc(getColl('transactions'), { amount: item.amount, description: item.title, type: 'expense', category: 'Bills', date: todayStr });
    if (item.type === 'bill') await updateDoc(doc(getColl('bills'), item.id), { lastPaidDate: new Date().toISOString() });
    else if (item.type === 'loan') { const loan = loans.find(l => l.id === item.id); if(loan) await updateDoc(doc(getColl('loans'), item.id), { pendingAmount: Math.max(0, loan.pendingAmount - loan.emiAmount), lastPaidDate: new Date().toISOString() }); }
    else if (item.type === 'due') await updateDoc(doc(getColl('dues'), item.id), { isPaid: true });
  };

  const handlePayAll = () => { pendingPayments.forEach(item => handlePayItem(item)); setShowPayPrompt(false); };
  
  const handleDownloadReport = () => {
    const headers = ["Date", "Description", "Type", "Category", "Amount"];
    const rows = transactions.map(t => [t.date, t.description, t.type, t.category, t.amount].join(","));
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(","), ...rows].join("\n");
    const link = document.createElement("a"); link.setAttribute("href", encodeURI(csvContent)); link.setAttribute("download", `MoneyManager_Report_${currentMonthName}.csv`); document.body.appendChild(link); link.click(); document.body.removeChild(link);
  };

  const handleSave = async (data) => {
    if (data.id && modalType !== 'savings_add' && modalType !== 'savings_withdraw') {
        const collName = modalType === 'bill' ? 'bills' : (modalType === 'loan' ? 'loans' : 'dues');
        await updateDoc(doc(getColl(collName), data.id), data);
    } else {
        if (modalType === 'transaction') { await addDoc(getColl('transactions'), {...data, date: new Date().toISOString().split('T')[0]}); if (data.type === 'income' && pendingPayments.length > 0) setTimeout(() => setShowPayPrompt(true), 500); }
        else if (modalType === 'bill') await addDoc(getColl('bills'), {...data, lastPaidDate: null});
        else if (modalType === 'loan') await addDoc(getColl('loans'), {...data, pendingAmount: parseFloat(data.totalAmount), lastPaidDate: null});
        else if (modalType === 'due') await addDoc(getColl('dues'), {...data, isPaid: false});
        else if (modalType === 'misc') await setDoc(doc(getColl('settings'), 'misc'), { ...miscSettings, limit: parseFloat(data.amount) });
        else if (modalType === 'savings_add') {
            const newSavings = (miscSettings.savings || 0) + parseFloat(data.amount);
            await setDoc(doc(getColl('settings'), 'misc'), { ...miscSettings, savings: newSavings });
        }
        else if (modalType === 'savings_withdraw') {
            const val = parseFloat(data.amount);
            if(val > (miscSettings.savings || 0)) { alert("Insufficient savings!"); return; }
            const newSavings = (miscSettings.savings || 0) - val;
            await setDoc(doc(getColl('settings'), 'misc'), { ...miscSettings, savings: newSavings });
            await addDoc(getColl('transactions'), { amount: val, description: 'Withdrawal from Savings', type: 'income', category: 'Savings', date: new Date().toISOString().split('T')[0] });
        }
    }
    setShowAddModal(false); setEditingItem(null);
  };

  const handleDelete = async (id, type) => { if(!window.confirm("Delete this item?")) return; const collName = type === 'transaction' ? 'transactions' : (type === 'bill' ? 'bills' : (type === 'loan' ? 'loans' : 'dues')); await deleteDoc(doc(getColl(collName), id)); };
  const handleEdit = (item, type) => { setEditingItem(item); setModalType(type); setShowAddModal(true); };
  const triggerSavingsAdd = () => { setModalType('savings_add'); setShowAddModal(true); };
  const triggerSavingsWithdraw = () => { setModalType('savings_withdraw'); setShowAddModal(true); };
  const handleLogout = async () => { await signOut(auth); setUser(null); };

  if (loadingAuth) return <div className="h-screen flex items-center justify-center bg-slate-50"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div></div>;
  if (!user) return <AuthScreen />;

  return (
    <div className="bg-slate-50 min-h-screen font-sans pb-24 md:max-w-md md:mx-auto md:border-x md:border-slate-200">
      <div className="sticky top-0 bg-white/80 backdrop-blur-md z-10 px-5 py-4 flex justify-between items-center border-b border-slate-100">
        <div className="flex items-center gap-2"><div className="bg-blue-600 p-2 rounded-xl"><Wallet className="w-5 h-5 text-white" /></div><span className="font-bold text-lg text-slate-800 tracking-tight">PocketMoney</span></div>
        <div className="flex items-center gap-3"><div className="relative"><Bell className="w-6 h-6 text-slate-400" />{pendingPayments.length > 0 && <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full border-2 border-white flex items-center justify-center text-[10px] font-bold text-white">{pendingPayments.length}</span>}</div><button onClick={handleLogout} className="p-1 bg-slate-100 rounded-full text-slate-500"><LogOut className="w-5 h-5"/></button></div>
      </div>
      <div className="px-5 py-6 space-y-6">
        {activeTab === 'home' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div className="grid grid-cols-1 gap-4">
                    <Card className="p-4 bg-gradient-to-r from-emerald-50 to-teal-50 border-emerald-100"><div className="flex justify-between items-center mb-3"><div className="flex items-center gap-2 text-emerald-700 font-bold"><PiggyBank className="w-5 h-5" /> Savings Pot</div><span className="text-2xl font-bold text-emerald-800">{formatCurrency(miscSettings.savings || 0)}</span></div><div className="flex gap-2"><button onClick={triggerSavingsWithdraw} className="flex-1 py-2 bg-white text-emerald-700 text-xs font-bold rounded-lg shadow-sm border border-emerald-100 active:scale-95">Withdraw</button><button onClick={triggerSavingsAdd} className="flex-1 py-2 bg-emerald-600 text-white text-xs font-bold rounded-lg shadow-lg shadow-emerald-200 active:scale-95">Add Money</button></div></Card>
                    <Card className="p-4"><div className="flex justify-between items-center mb-3"><div className="flex items-center gap-2 font-bold text-slate-700"><List className="w-4 h-4" /> Misc. Budget</div><button onClick={() => { setModalType('misc'); setShowAddModal(true)}} className="p-1 bg-slate-100 rounded-full text-slate-500 hover:text-blue-600"><Pencil className="w-3 h-3"/></button></div><div className="flex justify-between text-sm mb-1 text-slate-500"><span>Spent: {formatCurrency(miscSpent)}</span><span>Limit: {formatCurrency(miscSettings.limit || 0)}</span></div><div className="w-full bg-slate-100 rounded-full h-2 mb-3 overflow-hidden"><div className={`h-full rounded-full ${miscBalance < 100 ? 'bg-red-500' : 'bg-blue-500'}`} style={{ width: `${Math.min(100, (miscSpent / (miscSettings.limit || 1)) * 100)}%` }}></div></div><div className="flex justify-between items-center"><span className="font-bold text-slate-800">{formatCurrency(miscBalance)} left</span>{miscBalance > 0 && <button onClick={triggerSavingsAdd} className="px-3 py-1 bg-indigo-50 text-indigo-600 text-xs font-bold rounded-full flex items-center gap-1">Roll to Savings <ArrowRight className="w-3 h-3"/></button>}</div></Card>
                </div>
                <div><h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 flex justify-between items-center"><span>{currentMonthName} Commitments</span><span className="bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full text-[10px]">{pendingPayments.length} Left</span></h3>{pendingPayments.length === 0 ? <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-2xl flex items-center gap-3"><CheckCircle className="w-6 h-6 text-emerald-500" /><p className="text-emerald-800 text-sm font-medium">All bills & EMIs paid for {currentMonthName}!</p></div> : (<div className="space-y-3">{pendingPayments.map((item, idx) => (<Card key={`${item.type}-${item.id}-${idx}`} className="p-4 border-l-4 border-l-blue-500 flex flex-col justify-between"><div className="flex justify-between items-start mb-3"><div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-full flex items-center justify-center bg-blue-50 text-blue-600`}>{item.type === 'loan' ? <Landmark className="w-5 h-5"/> : (item.type === 'bill' ? <Zap className="w-5 h-5"/> : <Clock className="w-5 h-5"/>)}</div><div><p className="font-bold text-slate-800 text-sm">{item.title}</p><p className="text-xs text-slate-500">{item.daysLeft <= 0 ? <span className="text-red-500 font-bold">Due Now</span> : `In ${item.daysLeft} days`}</p></div></div><button onClick={() => handleEdit(item, item.type)} className="text-slate-300 hover:text-blue-600"><Pencil className="w-4 h-4"/></button></div><div className="flex items-center justify-between pt-2 border-t border-slate-50"><span className="font-bold text-slate-800 text-lg">{formatCurrency(item.amount)}</span><button onClick={() => handlePayItem(item)} className="bg-slate-900 text-white px-4 py-2 rounded-lg text-xs font-bold active:scale-95">Pay</button></div></Card>))}</div>)}</div>
                <div className="grid grid-cols-4 gap-2"><button onClick={() => { setModalType('transaction'); setEditingItem(null); setShowAddModal(true)}} className="flex flex-col items-center justify-center p-3 bg-blue-600 text-white rounded-2xl shadow-lg shadow-blue-200 active:scale-95 transition-transform"><Plus className="w-5 h-5 mb-1"/><span className="font-bold text-[10px]">Txn</span></button><button onClick={() => { setModalType('bill'); setEditingItem(null); setShowAddModal(true)}} className="flex flex-col items-center justify-center p-3 bg-white border border-slate-200 text-slate-600 rounded-2xl shadow-sm active:bg-slate-50 transition-transform"><Zap className="w-5 h-5 mb-1"/><span className="font-bold text-[10px]">Bill</span></button><button onClick={() => { setModalType('loan'); setEditingItem(null); setShowAddModal(true)}} className="flex flex-col items-center justify-center p-3 bg-white border border-slate-200 text-slate-600 rounded-2xl shadow-sm active:bg-slate-50 transition-transform"><Landmark className="w-5 h-5 mb-1"/><span className="font-bold text-[10px]">EMI</span></button><button onClick={() => { setModalType('due'); setEditingItem(null); setShowAddModal(true)}} className="flex flex-col items-center justify-center p-3 bg-white border border-slate-200 text-slate-600 rounded-2xl shadow-sm active:bg-slate-50 transition-transform"><Clock className="w-5 h-5 mb-1"/><span className="font-bold text-[10px]">Due</span></button></div>
            </div>
        )}
        {activeTab === 'reports' && (
        ...
        {activeTab === 'list' && (
             <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                <div className="flex p-1 bg-slate-200 rounded-xl"><button onClick={() => setModalType('transaction')} className={`flex-1 py-2 text-xs font-bold rounded-lg ${modalType === 'transaction' ? 'bg-white shadow-sm' : 'text-slate-500'}`}>History</button><button onClick={() => setModalType('bill')} className={`flex-1 py-2 text-xs font-bold rounded-lg ${modalType === 'bill' ? 'bg-white shadow-sm' : 'text-slate-500'}`}>Manage</button></div>
                {modalType === 'transaction' ? (
                    <div className="space-y-4">
                        <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input type="text" placeholder="Search transactions..." className="w-full pl-10 p-3 bg-white border border-slate-200 rounded-xl text-sm outline-none focus:border-blue-500" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/></div>
                        <div className="space-y-3">{filteredTransactions.map(t => (<Card key={t.id} className="p-4 flex items-center justify-between"><div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-full flex items-center justify-center ${t.type === 'income' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-600'}`}>{t.type === 'income' ? <TrendingUp className="w-5 h-5"/> : <TrendingDown className="w-5 h-5"/>}</div><div><p className="font-bold text-slate-800 text-sm">{t.description}</p><p className="text-xs text-slate-500">{t.date} • {t.category}</p></div></div><div className="flex flex-col items-end"><span className={`font-bold ${t.type === 'income' ? 'text-emerald-600' : 'text-slate-800'}`}>{t.type === 'income' ? '+' : '-'}{formatCurrency(t.amount)}</span><button onClick={() => handleDelete(t.id, 'transaction')} className="text-slate-300 text-xs mt-1"><Trash2 className="w-3 h-3"/></button></div></Card>))}</div>
                    </div>
                ) : (
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-6 py-3 flex justify-around items-center z-40 md:max-w-md md:mx-auto"><button onClick={() => setActiveTab('home')} className={`flex flex-col items-center gap-1 ${activeTab === 'home' ? 'text-blue-600' : 'text-slate-400'}`}><Home className="w-6 h-6" /><span className="text-[10px] font-bold">Home</span></button><button onClick={() => setActiveTab('reports')} className={`flex flex-col items-center gap-1 ${activeTab === 'reports' ? 'text-blue-600' : 'text-slate-400'}`}><PieChartIcon className="w-6 h-6" /><span className="text-[10px] font-bold">Report</span></button><button onClick={() => setActiveTab('list')} className={`flex flex-col items-center gap-1 ${activeTab === 'list' ? 'text-blue-600' : 'text-slate-400'}`}><List className="w-6 h-6" /><span className="text-[10px] font-bold">Manage</span></button></div>
      {showAddModal && (<Modal type={modalType} initialData={editingItem} close={() => { setShowAddModal(false); setEditingItem(null); }} save={handleSave} />)}
      {showPayPrompt && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200"><div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-200"><div className="text-center mb-6"><div className="w-16 h-16 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-4"><Wallet className="w-8 h-8" /></div><h2 className="text-xl font-bold text-slate-800">Income Added! 💰</h2><p className="text-slate-500 text-sm mt-2">You have <span className="font-bold text-slate-800">{formatCurrency(totalCommitments)}</span> in upcoming payments.</p></div><div className="bg-slate-50 rounded-xl p-4 mb-6 max-h-48 overflow-y-auto">{pendingPayments.map((r, i) => (<div key={i} className="flex justify-between items-center py-2 border-b border-slate-200 last:border-0"><div><p className="font-bold text-sm text-slate-700">{r.title}</p><p className="text-[10px] text-slate-400">{r.meta}</p></div><span className="font-bold text-slate-800">{formatCurrency(r.amount)}</span></div>))}</div><div className="flex flex-col gap-3"><Button onClick={handlePayAll} className="w-full bg-blue-600">Pay All Now ({formatCurrency(totalCommitments)})</Button><Button variant="ghost" onClick={() => setShowPayPrompt(false)} className="w-full">I'll pay later</Button></div></div></div>)}
    </div>
  );
}